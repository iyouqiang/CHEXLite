...
class FCMarketViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "行情"
        self.view.backgroundColor = COLOR_CellBgColor
        //self.adjuestInsets()
    ...
    func loadSegmentControl () {
        ...
        segmentControl.isMarket = true
        segmentControl?.setTitles(titles: self.marketTypes ?? [], fontSize: 15, normalColor: COLOR_MinorTextColor, tintColor: COLOR_White, showUnderLine: true)
        segmentControl.backgroundColor = COLOR_CellBgColor
        self.segmentContainer.addSubview(self.segmentControl!)
        self.view.addSubview(self.segmentContainer)
        
        let segLineVie = UIView()
        segLineVie.backgroundColor = COLOR_LineColor
        self.segmentContainer.addSubview(segLineVie)
        segLineVie.snp.makeConstraints { (make) in
    ...
    func loadMarketListView() {
        ...
        let marketScroller: UIScrollView = UIScrollView.init(frame: CGRect(x: 0, y: kNAVIGATIONHEIGHT + 50, width: kSCREENWIDTH, height: (kSCREENHEIGHT-kTABBARHEIGHT - 100 - KSTATUSBARHEIGHT)))
