    ...
    private func initSubviews() {
        ...
        quickNavView = FCQuickNavView.init(items: items, clickItemAction: {[weak self] (itemModel, index) in
            ...
            if (index == 0) {
                ...
                let webVC = PCWKWebHybridController.init(url: URL(string: FCNetAddress.netAddresscl().hosturl_NEWGUIDE))!
                webVC.hidesBottomBarWhenPushed = true
                self?.parentVC?.navigationController?.pushViewController(webVC, animated: true)
                ...
                FCUserInfoManager.sharedInstance.loginState { (model) in
                    let webVC = PCWKWebHybridController.init(url: URL(string: FCNetAddress.netAddresscl().hosturl_TRADE))!
                    webVC.hidesBottomBarWhenPushed = true
                    self?.parentVC?.navigationController?.pushViewController(webVC, animated: true)
                }
                ...
                let webVC = PCWKWebHybridController.init(url: URL(string: FCNetAddress.netAddresscl().hosturl_CPEEXCHANGE))!
                webVC.hidesBottomBarWhenPushed = true
                self?.parentVC?.navigationController?.pushViewController(webVC, animated: true)
            }
        })
        ...
        agreeMentView.snp.makeConstraints { (make) in
            ...
            make.left.right.equalToSuperview()
            make.top.equalTo(self.mainSymbolsView.snp.bottom).offset(15)
        ...
        line3View.backgroundColor = COLOR_BGColor
...
extension FCHomeCompositeView {
    ...
    func configureAction() {
        ...
        agreeMentView.tradingSkillBlock = {
            ...
            let webVC = PCWKWebHybridController.init(url: URL(string: FCNetAddress.netAddresscl().hosturl_TRADESKILL))!
            webVC.hidesBottomBarWhenPushed = true
            self?.parentVC?.navigationController?.pushViewController(webVC, animated: true)
